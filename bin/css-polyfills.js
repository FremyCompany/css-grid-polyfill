...
module.exports = (function(window, document) { "use strict";
	...
	function EventStream(connect, disconnect, reconnect) {
		...
		// global variables
		var callback=null;
		var yieldEvent = function(a0) {
		
			if(window.trackEvents) {
				cssConsole.log(this);
				cssConsole.log(callback);
				cssConsole.log(a0);
				window.trackEvents = false;
			}
	...
	if("MutationObserver" in window) {
		DOMUpdateEventStream = function DOMUpdateEventStream(options) {
			...
			// start the event stream
			var observers = (window.observers = window.observers || []);
			var observer = null;
			EventStream.call(
				this, 
				function connect(yieldEvent) { var ste = {}; try { throw new Error() } catch(ex) { ste=ex; }; if(config) { observer=new MutationObserver(yieldEvent); observer.observe(target,config); observer.target=target; observer.config=config; observer.stack = ste.stack; observer.yieldEvent = yieldEvent; console.log('connect', observer); observers.push(observer); target=null; config=null; } },
				function disconnect() { console.log('disconnect', observer); /*observer && observer.disconnect(); observer=null;*/ },
				function reconnect() { console.log('reconnect', observer); /*observer.takeRecords();*/ }
			);
